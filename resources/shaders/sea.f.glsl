...
vec3 calc_normal(vec3 c, vec3 r, vec3 l, vec3 u, vec3 d){
	vec3 c2r = normalize(r - c);
	vec3 c2l = normalize(l - c);
	vec3 c2u = normalize(u - c);
	vec3 c2d = normalize(d - c);

	vec3 norm = c2r + c2l + c2u + c2d;
	if(norm.y < 0.0f){
		norm *= -1.0f;
	}
	return norm;
...
const float dist_neighbor = 498.0f / 500.0f / 500.0f;
...
void main(){
	...
	}else if(u_b_heightmap){
		normal_direction = calc_normal(
			vec3(          0.0f,                                                       0.0f,           0.0f),
			vec3(-1.0f / 500.0f, get_height(c_v_tex + vec2( dist_neighbor,           0.0f)),           0.0f),
			vec3( 1.0f / 500.0f, get_height(c_v_tex + vec2(-dist_neighbor,           0.0f)),           0.0f),
			vec3(          0.0f, get_height(c_v_tex + vec2(          0.0f,  dist_neighbor)), -1.0f / 500.0f),
			vec3(          0.0f, get_height(c_v_tex + vec2(          0.0f, -dist_neighbor)),  1.0f / 500.0f)
		);
		normal_direction *= u_m_normal;
	}else{
